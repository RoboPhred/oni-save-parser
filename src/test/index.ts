...
import { ContainerModule, injectable } from "microinject";
...
import { ParseStepListener, ParseStepEventArgs } from "../parse-steps";
...
function testWriteback() {
  ...
  writeFileSync(
    `./test-data/${fileName}-writeback.sav`,
...
function testDumpJson() {
    ...
    for (let gameObject of items) {
      ...
      for (let behavior of gameObject.behaviors) {
        ...
        if (seenBehaviors.has(behavior.name)) {
          ...
          console.log(
            `Seen behavior "${behavior.name}" more than once in "${category}"`
          );
          continue;
        }
  ...
  writeFileSync(
    `./test-data/${fileName}-gameObjects.json`,
