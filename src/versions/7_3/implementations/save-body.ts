...
export class SaveBodyInstanceImpl implements SaveBodyInstance {
  ...
  write(writer: DataWriter): void {
    if (this._header.isCompressed) {
      ...
      const compressedBytes = this._stepExecutor.do("compressing", () => {
  ...
  private _parseState(reader: DataReader) {
    ...
    this._stepExecutor.do("header", () => {
      ...
      if (versionMajor !== expectedMajor) {
        throw new Error(
          `Failed to parse SaveBody: Version mismatch: Expected major version ${expectedMajor} but got ${versionMajor}.`
        );
      }
      ...
      if (versionMinor > expectedMinor) {
        ...
        if (this._logger)
          this._logger.warn(
            `SaveBody version ${versionMajor}.${versionMinor} has a higher minor version than expected ${expectedMajor}.${expectedMinor}.  Problems may occur with parsing.`
          );
      }
  ...
  fromJSON(value: SaveBody): void {
    if (value.versionMajor !== SaveBodyInstanceImpl.CURRENT_VERSION_MAJOR) {
      throw new Error(
        `Failed to parse SaveBody: Version mismatch: Expected major version ${
          SaveBodyInstanceImpl.CURRENT_VERSION_MAJOR
        } but got ${value.versionMajor}.`
      );
    }
  ...
  toJSON(): SaveBody {
    if (!this._versionMinor) {
      throw new Error(
        `Failed to serialize SaveBody: Data has not been loaded.`
      );
    }
