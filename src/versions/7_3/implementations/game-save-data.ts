...
import { validateDotNetIdentifierName, ensureNotNull } from "../../../utils";

import { DataReader, DataWriter } from "../../../binary-serializer";

import { TypeTemplateSerializer } from "../type-serializer";

import { GameSaveDataInstance, SaveGameScope } from "../services";

import { GameSaveData } from "../interfaces";
...
export class GameSaveDataInstanceImpl implements GameSaveDataInstance {
  ...
  parse(reader: DataReader): void {
    ...
    if (typeName !== AssemblyTypeName) {
      throw new Error(
        `Failed to parse GameSaveData: Expected to find type name "${AssemblyTypeName}", but got "${typeName}".`
      );
    }
    this._data = this._templateSerializer.parseTemplatedType<GameSaveData>(
      reader,
      AssemblyTypeName
    );
    if (!this._data) {
      throw new Error(
        "Failed to parse GameSaveData: Parsed template type data was a null value."
      );
    }
  }
  ...
  write(writer: DataWriter): void {
    if (!this._data) {
      throw new TypeError(
        "Failed to write GameSaveData: Data has not been parsed."
      );
    }
  ...
  toJSON() {
    if (!this._data) {
      throw new Error(
        "Failed to serialize GameSaveData json: Data has not been set."
      );
    }
    return { ...this._data };
  }
