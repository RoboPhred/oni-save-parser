import { injectable, inject, provides, inScope } from "microinject";
...
import { TypeTemplate, TypeTemplateMember } from "./interfaces";
...
export class TypeTemplateRegistryImpl
  implements TypeTemplateRegistry, TypeTemplateSerializer {
  ...
  @inject(TypeSerializer) private _typeSerializer: TypeSerializer | undefined;
  ...
  parse(reader: DataReader): void {
    ...
    this._stepExecutor.for("type-templates", templateCount, index => {
      ...
      if (this._templates.has(name)) {
        throw new Error(
          `Failed to parse type template: A template named "${name}" already exists.`
        );
      }
  ...
  write(writer: DataWriter): void {
    ...
    this._stepExecutor.forEach(
      ...
      templateName => {
        ...
        if (!template) {
          throw new Error(
            `Failed to write type template: A template in templateNameOrderings was not in the template map.`
          );
        }
