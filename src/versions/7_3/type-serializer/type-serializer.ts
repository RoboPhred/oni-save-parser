...
export class TypeSerializerImpl
  implements TypeSerializer, TypeDescriptorSerializer {
  ...
  parseDescriptor(reader: DataReader): TypeDescriptor {
    ...
    if (!type) {
      throw new Error(
        `Failed to parse type descriptor: Type ID ${id} has no registered serialization info.`
      );
    }
  ...
  writeDescriptor(writer: DataWriter, descriptor: TypeDescriptor): void {
    ...
    if (!type) {
      throw new Error(
        `Failed to write type descriptor: Descriptor name ${name} has no registered serialization info.`
      );
    }
  ...
  private _buildCache() {
    ...
    for (let info of this._serializerInfos) {
      ...
      if (lastIdDefiner !== undefined) {
        throw new Error(
          `Duplicate serializer type info ID: ${info.id} from name "${
            info.name
          }" (defined by "${lastIdDefiner.name}").`
        );
      }
