...
let OniSaveRootImpl = class OniSaveRootImpl {
    ...
    parse(reader) {
        ...
        if (rootName !== exports.SaveFileRoot) {
            throw new Error(`Expected to find "${exports.SaveFileRoot}", but got "${rootName}"`);
        }
        this._saveFileRoot = this._templateSerializer.parseTemplatedType(reader, exports.SaveFileRoot);
    }
    write(writer) {
        if (!this._saveFileRoot) {
            throw new Error("Failed to write SaveFileRoot: No root loaded.");
        }
        writer.writeKleiString(exports.SaveFileRoot);
        this._templateSerializer.writeTemplatedType(writer, exports.SaveFileRoot, this._saveFileRoot);
    }
    toJSON() {
        const streamed = {};
