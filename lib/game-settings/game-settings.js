...
let OniGameSettingsImpl = class OniGameSettingsImpl {
    ...
    parse(reader) {
        ...
        if (rootName !== assembly_types_1.GameSettings) {
            throw new Error(`Expected to find "${assembly_types_1.GameSettings}", but got "${rootName}"`);
        }
        this._settings = this._typeSerializer.parseTemplatedType(reader, assembly_types_1.GameSettings);
    }
    write(writer) {
        if (!this._settings) {
            throw new Error("Failed to write GameSettings: No game settings loaded.");
        }
        writer.writeKleiString(assembly_types_1.GameSettings);
        this._typeSerializer.writeTemplatedType(writer, assembly_types_1.GameSettings, this._settings);
    }
    toJSON() {
        return {
