...
const behavior_parser_1 = require("./behavior-parser");
function* parseGameObject(templateParser) {
    ...
    for (let i = 0; i < behaviorCount; i++) {
        behaviors[i] = yield* behavior_parser_1.parseGameObjectBehavior(templateParser);
    }
    const gameObject = {
        position,
        rotation,
...
function* unparseGameObject(gameObject, templateUnparser) {
    ...
    for (const behavior of behaviors) {
        yield* behavior_parser_1.unparseGameObjectBehavior(behavior, templateUnparser);
    }
}
exports.unparseGameObject = unparseGameObject;
//# sourceMappingURL=game-object-parser.js.map