...
const CURRENT_VERSION_MINOR = 12;
function validateVersion(major, minor) {
    if (major !== CURRENT_VERSION_MAJOR || minor !== CURRENT_VERSION_MINOR) {
        throw new Error(`Save version "${major}.${minor}" is not compatible with this parser.  Expected version "${CURRENT_VERSION_MAJOR}.${CURRENT_VERSION_MINOR}".`);
    }
}
