...
let GameSaveRootInstanceImpl = class GameSaveRootInstanceImpl {
    constructor(_templateSerializer, _stepExecutor) {
    ...
    parse(reader) {
        ...
        this._data = this._stepExecutor.do("save-file-root", () => this._templateSerializer.parseTemplatedType(reader, AssemblyTypeName));
    }
    write(writer) {
        if (!this._data) {
        ...
        this._stepExecutor.do("save-file-root", () => this._templateSerializer.writeTemplatedType(writer, AssemblyTypeName, this._data));
    }
    fromJSON(value) {
