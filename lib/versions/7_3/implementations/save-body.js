...
let SaveBodyInstanceImpl = SaveBodyInstanceImpl_1 = class SaveBodyInstanceImpl {
    constructor(_header, _saveRoot, _gameSettings, _gameObjectManager, _gameData, _stepExecutor, _logger) {
    ...
    parse(reader) {
        if (this._header.isCompressed) {
            const deflatedReader = this._stepExecutor.do("decompressing", () => new binary_serializer_1.ZlibDataReader(reader.viewAllBytes()));
    ...
    write(writer) {
        if (this._header.isCompressed) {
            const rawWriter = new binary_serializer_1.ArrayDataWriter();
...
SaveBodyInstanceImpl = SaveBodyInstanceImpl_1 = __decorate([
    ...
    __param(5, microinject_1.inject(parse_steps_1.ParseStepExecutor)),
    __param(6, microinject_1.inject(logging_1.Logger)), __param(6, microinject_1.optional())
